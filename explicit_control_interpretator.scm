...
ev-application
	...
	(test (op self-evaluating?) (reg unev))
...
ev-application-symbol-lookup
	(assign continue (label ev-application-symbol-after-lookup)) 
    (goto (label eval-dispatch)) 

ev-application-symbol-after-lookup
	(assign unev (op operands) (reg exp))
	(assign proc (op operator) (reg val))
